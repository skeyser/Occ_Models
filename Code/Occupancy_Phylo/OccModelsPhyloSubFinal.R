...
#           logit( p[s,j,k] ) <- int.p[s] + alpha[1] * Obs.ma[j,k] +
#                                 alpha[2] * Ord.ma[j,k] +
#                                 alpha[3] * TOD.ma[j, k]
...
#     for( s in 1:S ){
#       int.p[s] <- log(mean.p[s] / (1 - mean.p[s]))
#       mean.p[s] ~ dbeta(4, 4)
#     } #S
# 
...
#     sigma.psi ~ dt(0, 1, 4) T(0, 10) #Variance of ps
# 
#     #random route effect
...
############################################################
###Model with alpha[4] removed, so mass has been excluded###
############################################################
#Restrictions put on detection mod bit and loosening########
#up the occupancy ##########################################
...
################ end of model specification  #####################################
modelname <- "pm3.txt" #changed model name to phylomodel2 
#create initial values
zst <- ydf
zst[is.na( zst) ] <- 1
inits <- function(){ list( z = zst ) }
...
upm3 <- autojags( win.data, inits = inits, params, modelname,
                  ...
                  iter.increment = 5000, max.iter = 500000,
                  Rhat.limit = 1.3, save.all.iter=FALSE, parallel = TRUE )
