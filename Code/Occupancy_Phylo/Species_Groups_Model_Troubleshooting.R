#################################################################
###########Script for vsiualizing species groupings##############
#################################################################
...
final_sp_df <- read.csv(here::here("Data_BBS/Generated DFs/Final_Sp_Df_DetectionsOnlyReclass.csv"))
...
species.info <- final_sp_df %>% select(c(sci_name, spp.id, BodyMass, Nocturnal,
                                         ...
                                         ForStrat_aerial, Phylo.V1))
species.info$Detected <- 1
species.info$sci_name <- as.character(species.info$sci_name)
species.info <- species.info %>% group_by(sci_name) %>% mutate(Occurences = sum(Detected))
...
group.names <- unique(final_sp_df$Phylo.V1)
group.names <- as.character(group.names)
par(mfrow = c(2, 2))


for (i in 1:length(unique(group.names))){
  data.temp <- species.info[species.info$Phylo.V1 == group.names[i],]
  plots <- barplot(data.temp$Occurences ~ data.temp$spp.id, las =2,
                   xlab = NA,
                   main = print(group.names[i]))
}
