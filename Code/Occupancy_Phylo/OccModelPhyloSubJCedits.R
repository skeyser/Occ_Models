...
####From om5.text with species random effects#####
##############################################################################
######### Same as pm2 but trying to not calculate parameters for     #########
###### segments that were not sampled   version 1                        #######
##############################################################################
########Model specification###############
sink( "pm3.txt" )
cat("
...
##############################################################################
######### Same as pm2 but trying to not calculate parameters for     #########
###### segments that were not sampled   version 2                        #######
##############################################################################
########Model specification###############
sink( "pm4.txt" )
cat("
    ...
    muz[ s, j, k ] <- psi[ j, k ] * w.bcr[ s, j, k ]
    #only estimate z for surveyed sites and segments
    muzsurv[ s, j, k ] <- step( ydf[ s, j, k ] ) * muz[ s, j, k ]
    z[ s, j, k ] ~ dbern( muz[ s, j, k ] )            
    }#S
    }#K
    }#J
    ...
    delta[ s ]
    #only use p for surveyed sites
    psurv[ s, j, k ] <- step( ydf[ s, j, k ] ) * p[ s, j, k ]
    mup[ s, j, k ] <- z[s, j, k] * psurv[s, j, k]
    #relate observed occupancy to detection prob and estimated occupancy
    ydf[s, j, k] ~ dbern( mup[ s, j, k ]  )
    }#K
    ...
    #only use omega for undetected species:
#    omega.y[ s, j, k ] <- ifelse( det[ s, j, k ] == 1, 1, omega )
    omega.y[ s, j, k ] <- equals( ydf[ s, j, k ], 1 ) * omega 
    } #k
    } #j
    } #s
