...
final_sp_df$unique_id <- paste0(final_sp_df$rteno.x, "_", final_sp_df$Year)
...
bbs_total <- bbs_total %>% dplyr::select(rteno.x, rt_yr, Year,
                                         BCR, unique_id) #Selection variables 
...
bbs_total$site <- as.character(bbs_total$rteno.x)
...
#Merge split sites for complete time series
# occ$rteno.x <- gsub(pattern = "25080", replacement = "25180", occ$rteno.x)
# occ$site <- gsub(pattern = "25080", replacement = "25180", occ$site)
# occ$unique_id <- gsub(pattern = "25080", replacement = "25180", occ$unique_id)
# #
# occ$rteno.x <- gsub(pattern = "25016", replacement = "25116", occ$rteno.x)
# occ$site <- gsub(pattern = "25016", replacement = "25116", occ$site)
# occ$unique_id <- gsub(pattern = "25016", replacement = "25116", occ$unique_id)
# #
# occ$rteno.x <- gsub(pattern = "42006", replacement = "42106", occ$rteno.x)
# occ$site <- gsub(pattern = "42006", replacement = "42106", occ$site)
# occ$unique_id <- gsub(pattern = "42006", replacement = "42106", occ$unique_id)

#Merge it with the year bins 
occ <- merge(occ, Bins, by = "Year")
occ$unique_id <- paste0(occ$rteno.x, "_", occ$Year)
...
slopes_sites <- data.frame(site.list, slope = NA, slope50 = NA, slope.wet = NA) #slope.mcmc = NA

bbs_div_means <- bbs_total
bbs_div_means <- bbs_div_means %>% left_join(occSR, by = "unique_id") #left_join(sr.mcmc, by = "unique_id") %>%
...
for (i in 1:n.sites){
  ...
  if (nrow(site_total_temp) > 1){
    ...
    slopes_sites[i, 3] <- slope.temp3
    slopes_sites[i, 4] <- slope.temp4
  }
}
...
slopes_sites <- slopes_sites %>% dplyr::select(site, slope, slope50, slope.wet) #slope.mcmc

#Mean alpha at the site level 
bbs_div_means <- bbs_div_means[, unique(colnames(bbs_div_means))]
site_means <- bbs_div_means %>% dplyr::select(Site_div, SR50, SR.wet, site) #SR_MCMC,
...
site_alpha <- bbs_div_means %>% dplyr::select(Site_div, SR50, SR.wet, unique_id) #SR_MCMC, SD_MCMC
...
for (n in 1:n.sites){
  site.temp <- site.list[n]
  #Find all the years for which that site was surveys
  yrs.temp <- unique(occ[which(occ$rteno.x == site.temp),"Year"])
  yrs.temp <- yrs.temp[yrs.temp >= 1]
  #only consider sites that were observed in more than 5 years 
  if (length(yrs.temp) > 1){ #select all sites that have mre than 1 year bin 
    #Create a baseline year from years temp
...
bbs_bin <- bbs_total %>% dplyr::select(rteno.x, site, BCR, Site_div.x, rtename, Yr_bin, beta.turn, beta.nest, beta.jac, 
  ...
  group_by(site, Yr_bin) %>% summarize(
    ...
    alpha = mean(Site_div.x), alpha50 = mean(SR50), alpha.wet = mean(SR.wet), #beta.mcmc = mean(beta.mcmc), alpha.mcmc = mean(SR_MCMC),
  )
...
#Save BBS_Df so I dont have to continuously run script
bbs_total <- bbs_total %>% dplyr::select(-c("Site_div.y"))
colnames(bbs_total)[colnames(bbs_total) == "Site_div.x"] <- "Site_div"

#write.csv(bbs_total, here::here("Data_BBS/Generated DFs/DiversityMatrixRtLvl.csv"))
...
prism.seg <- tidyr::separate(prism.seg, col = "Site", into = c("Site", "segment"), by = "_")

prism.seg <- prism.seg %>% group_by(Site, Year, Month) %>% summarize_if(is.numeric, mean)
...
prism.seg.means <- prism.seg.means %>% separate(col = unique_id, into = c("Site", "Year"))
...
seg.climate <- seg.climate[, c("Site", "Year", "tmean.c", "tmax.c", "tmin.c", "pmean.c",
...
mod.clim <- lm(data = gom.climate.test, tmean_Spring.c ~ Year)
summary(mod.clim)
Anova(mod.clim)

ggplot(data = gom.climate, aes(x = Year, y = tmax.c, group = 1)) + geom_point() + geom_smooth()
...
seg.climate <- seg.climate %>% dplyr::select(-c("index.yr.min", "index.yr.max"))
seg.climate <- merge(seg.climate, Bins, by = "Year")
seg.climate <- seg.climate %>% dplyr::select(-c("Yr_bin.x")) %>% rename(Yr_bin = Yr_bin.y)
...
#write.csv(bbs_clim, here::here("Data_BBS/Generated DFs/DF_div_clim_rtlvl.csv"))
#######################################################################################################
...
bbs_lulc$unique_id <- paste0(bbs_lulc$rteno, "_", bbs_lulc$Yr_bin)

lulc <- bbs_lulc %>% dplyr::select(c("unique_id", "Year", "Background", "mangrove", "Urban", "Ag",
                                      "Grassland", "Forest", "Woody_Wetlands", "Emergent_Wetlands",
                                      "Bare", "Water", "rteno"))

lulc <- lulc %>% group_by(unique_id) %>% summarize_if(is.numeric, sum) %>%
        mutate(total_cover = Background + Urban + Ag + Grassland + Forest + Woody_Wetlands + Emergent_Wetlands + Bare + Water,
               total_cover_nb = Urban + Ag + Grassland + Forest + Woody_Wetlands + Emergent_Wetlands + Bare + Water,
               total_wet = Emergent_Wetlands + Woody_Wetlands, pct_wetland = total_wet / total_cover_nb)

bbs_full <- merge(lulc, bbs_clim, by = "unique_id")


#Calculate the % of Emergent, Woody Wetlands, and Urban
bbs_full <- bbs_full %>% group_by(rteno) %>%
...
bbs_full <- bbs_full %>% group_by(rteno) %>% mutate(alpha.change = Site_div - first(Site_div),
                                                   alpha50.change = SR50 - first(SR50), alphawet.change = SR.wet - first(SR.wet), alpha.pchange = ((Site_div - first(Site_div)) / first(Site_div)),
                                                   alpha50.pchange = ((SR50 - first(SR50)) / first(SR50)), alphawet.pchange = ((SR.wet - first(SR.wet)) / first(SR.wet)))
...
mod2 <- lm(data = bbs_last, beta50.nest ~ SR50 + mean.anom.bird + p.anom + diff.from.first.man + diff.from.first.wwnm + diff.from.first.ur + Duration)
summary(mod2)

mod3 <- lm(data = bbs_last, beta50.turn ~ SR50 + mean.anom.bird + p.anom + diff.from.first.man + diff.from.first.wwnm + diff.from.first.ur + Duration)
summary(mod3)

mod4 <- lm(data = bbs_last, alpha50.pchange ~ mean.anom.bird + p.anom + diff.from.first.man + diff.from.first.wwnm + diff.from.first.ur + Duration)
summary(mod4)

 mod5 <- lm(data = bbs_last, alphawet.pchange ~ mean.anom.bird + p.anom + diff.from.first.man + diff.from.first.wwnm + diff.from.first.ur + Duration)
summary(mod5)

mod6 <- lm(data = bbs_last, alpha.pchange ~ mean.anom.bird + p.anom + diff.from.first.man + diff.from.first.wwnm + diff.from.first.ur + Duration)
summary(mod6)
...
cmrl.occ <- aggregate(data = present, Latitude ~ rteno.x + Year, FUN = mean)
lm.cmrl <- lmer(data = cmrl.occ, Latitude ~ Year + (1|rteno.x), REML = F)
summary(lm.cmrl)
Anova(lm.cmrl)
...
bbs_last <- bbs_last %>% rename(SR = Site_div, min.yr = min.yr.x, max.yr = max.yr.x, min.yr.bin = min.yr.bin.x, max.yr.bin = max.yr.bin.x, 
  ...
  dplyr::select(c("unique_id", "rteno", "Site", "Year", "Yr_bin", "rt_yr", "min.yr", "max.yr", "min.yr.bin", "max.yr.bin",
                  ...
                  "alpha50.change", "alphawet.change", "alpha.pchange", "alpha50.pchange", 
                  "alphawet.pchange", "SR", "SR50", "SR.wet", "tmean.c", "tmax.c", "tmin.c", "pmean.c", 
